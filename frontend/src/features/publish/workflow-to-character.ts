...
function getNode(workflow: WorkflowRun, type: string): WorkflowNode | null {
  return workflow.nodes.find((s) => s.type === type) ?? null
}

/** 从 character-template 节点提取母版 URL */
function extractCharacterTemplateUrl(workflow: WorkflowRun): string | null {
  const template = getNode(workflow, 'character-template')
...
function extractCandidates(workflow: WorkflowRun): CharacterTemplateCandidate[] {
  const node = getNode(workflow, 'character-template')
...
function extractDescription(workflow: WorkflowRun): string {
  const node = getNode(workflow, 'character-setup')
...
function extractReviewedActions(workflow: WorkflowRun) {
  return workflow.nodes.flatMap((node) => {
    const review = workflow.nodes.find(
      (candidate) =>
        candidate.type === 'review' &&
        !candidate.deletedAt &&
        candidate.dependsOnNodeIds.includes(node.id),
    )
...
export function workflowRunToCharacter(run: WorkflowRun): Character | null {
  const templateNode = getNode(run, 'character-template')

  // 至少需要母版已生成
  if (!templateNode || templateNode.status !== 'passed') return null

  const characterTemplateUrl = extractCharacterTemplateUrl(run)
  const candidates = extractCandidates(run)
  const description = extractDescription(run)
  const reviewedActions = extractReviewedActions(run)
