...
import { getActiveNode, replaceWorkflowNode, type WorkflowNodeTarget } from './workflow-state'
...
export interface CharacterTemplateTask {
  start(runId: WorkflowRun['id'], target: WorkflowNodeTarget): Promise<WorkflowRun>
...
export function createCharacterTemplateTask({
  ...
  async function start(runId: WorkflowRun['id'], target: WorkflowNodeTarget): Promise<WorkflowRun> {
  ...
  function submit(runId: WorkflowRun['id'], target: WorkflowNodeTarget) {
    ...
    const submission = performSubmission(runId, target).finally(() => submissions.delete(key))
  ...
  async function performSubmission(
    ...
    await save(
      replaceWorkflowNode(before, beforeNode.id, (current) =>
        current.type === 'character-template' ? { ...current, submissionId } : current,
      ),
    )
    ...
    try {
      ...
      await markFailed(runId, target, null, submissionId, errorMessage(cause, '角色图生成请求失败'))
      throw cause instanceof Error ? cause : new Error(String(cause))
    }
  }

  function ensureTaskSubscription(run: WorkflowRun, nodeId: WorkflowNode['id'], taskId: string) {
    ...
    try {
      const stop = generationApis.subscribe(run.projectId, taskId, (event) => {
        void handleGenerationEvent(run.id, { runId: run.id, nodeId }, taskId, event).catch(
          (cause) => {
            console.error('[workflow] 保存角色图生成终态失败，正在重新查询', cause)
            void generationApis
              .get(run.projectId, taskId)
              .then((task) =>
                handleGenerationEvent(run.id, { runId: run.id, nodeId }, taskId, taskEvent(task)),
              )
              .catch((retryCause) => {
  ...
  async function handleGenerationEvent(
    ...
    if (event.taskId !== taskId || event.status === 'pending' || event.status === 'running') return
    if (event.status === 'failed') {
      await markFailed(runId, target, taskId, null, event.error?.trim() || '角色图生成任务失败')
      return
    }
    if (event.type !== 'character_image') {
      await markFailed(runId, target, taskId, null, '任务结果类型与角色图生成节点不匹配')
      return
    }
  ...
  async function resume(runId: WorkflowRun['id']): Promise<WorkflowRun | null> {
    ...
    if (activeNode?.type !== 'character-template' || activeNode.status !== 'active') return run
  ...
  async function applyServerResult(
    ...
    const candidateStep = run.nodes.find((item) => item.type === 'action-first-frame')
    if (!candidateStep) throw new Error('WorkflowRun 缺少 action-first-frame 节点')
    ...
    const updated: WorkflowRun = {
      ...
      nodes: run.nodes.map((current) => {
        ...
        if (current.id === candidateStep.id && current.type === 'action-first-frame') {
  ...
  async function markFailed(
    ...
    if (
      ...
      (expectedSubmissionId !== null && node.submissionId !== expectedSubmissionId)
    ) {
      return run
    }
    ...
    if (node.taskId) stopSubscription(subscriptionKey(run.id, node.id, node.taskId))
...
function parseCharacterImageOutput(value: unknown): CharacterImageOutput | null {
  if (!value || typeof value !== 'object' || !('type' in value) || !('imageUrls' in value)) {
    return null
  }
  if (value.type !== 'character_image' || !Array.isArray(value.imageUrls)) return null
...
function subscriptionKey(runId: WorkflowRun['id'], nodeId: WorkflowNode['id'], taskId: string) {
