...
describe('WorkflowRun first vertical slice', () => {
  it('runs character setup through a completed character-template task', async () => {
    ...
    expect(inFlight?.nodes.find((node) => node.type === 'character-template')).toMatchObject({
      status: 'active',
    ...
    await vi.waitFor(async () => {
      ...
      expect(completed?.nodes.find((node) => node.type === 'character-template')).toMatchObject({
        status: 'passed',
        taskId: null,
        output: {
          type: 'character_image',
      ...
      expect(completed?.nodes.find((node) => node.type === 'action-first-frame')).toMatchObject({
        status: 'active',
      })
    })
  })
