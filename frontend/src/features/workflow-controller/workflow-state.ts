...
/**
 * 采用用户已上传的角色母版，明确跳过角色图片生成和候选选择。
 *
 * 该转换不创建 Generation 任务；上传图片只作为已提供的输入和已确认母版记录。
 * 动作任务仍由随后的 startActionGeneration 正常提交。
 */
export function acceptUploadedCharacterTemplateState(
  ...
  if (!activeStep || activeStep.type !== 'character-setup') {
    throw new Error('当前只能在角色资料步骤采用上传母版')
  }

  const normalizedUrl = String(templateUrl).trim()
  if (!normalizedUrl) throw new Error('上传角色母版引用不能为空')
  const mediaReference = normalizedUrl as MediaReference
