    ...
    <aside aria-label="资产检查器" className="space-y-4">
      <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-xs">
          ...
          <dt className="text-slate-500">QC</dt>
          <dd className="font-medium text-slate-900">{frame?.qc ?? "—"}</dd>
          ...
          {inspectionStatus !== null ? (
            <>
              ...
              <dd className="font-medium text-slate-900">
                {inspectionStatus === "passed" ? "通过" : "发现问题"}
              </dd>
            </>
          ) : null}
          {source !== null ? (
            <>
              <dt className="text-slate-500">流程来源</dt>
              <dd className="break-all font-medium text-slate-900">
                {source.runId} / {source.revisionId}
              </dd>
            </>
          ) : null}
        </dl>
