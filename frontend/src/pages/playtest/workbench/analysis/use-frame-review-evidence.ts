...
import type { PlaytestActionType, PreviewSequence } from "../model/types";
...
export function useFrameReviewEvidence(
  ...
  const frameDescriptors =
    sequence?.frames.map((frame) => ({ imageUrl: frame.imageUrl, rootMotion: frame.rootMotion })) ??
    [];
  const imageUrls = frameDescriptors.map((frame) => frame.imageUrl);
  const sequenceKey =
    sequence === null || actionType === null
      ? null
      : JSON.stringify([actionType, ...frameDescriptors]);
