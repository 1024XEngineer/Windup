...
describe("playtest visual primitives", () => {
  it("renders prop-supplied action names, statuses and actual frame counts while disabling empty actions", () => {
    ...
    expect(screen.getByRole("button", { name: /行走/ }).textContent).toContain("confirmed");
  ...
  it("surfaces frame markers and delegates timeline selection without its own playback behavior", () => {
    // Catches a timeline dropping QC/rejection annotations or mutating playback rather than using its selection callback.
    ...
    expect(screen.getByText("QC: failed")).toBeTruthy();
