...
describe("playtest visual primitives", () => {
  it("renders prop-supplied action names, FPS and actual frame counts while disabling empty actions", () => {
    ...
    expect(screen.getByRole("button", { name: /行走/ }).textContent).toContain(
      "12 FPS",
    );
    expect(screen.getByRole("button", { name: /行走/ }).textContent).toContain(
      "2 帧",
    ...
    expect(
      (screen.getByRole("button", { name: /空动作/ }) as HTMLButtonElement)
        .disabled,
    ).toBe(true);
  ...
  it("reports horizontal travel from the measured stage and actor widths", () => {
    ...
    const rect = vi
      ...
      .mockImplementation(function (this: HTMLElement) {
        const width =
          this.getAttribute("aria-label") === "动画预览舞台" ? 600 : 200;
        return {
          width,
          height: 400,
          x: 0,
    ...
    expect(onHorizontalBoundsChange).toHaveBeenLastCalledWith({
      minX: -200,
      maxX: 200,
    });
    rect.mockRestore();
