...
import {
  type PlaytestKeyboardCommands,
  usePlaytestKeyboard,
} from "./use-playtest-keyboard";
...
function mountKeyboard(
  commands: PlaytestKeyboardCommands,
  enabled = true,
): () => void {
...
describe("usePlaytestKeyboard", () => {
  ...
  it.each([
    ...
    "does not intercept any shortcut when focus is in %s",
    (_label, createElement) => {
      ...
      const events = interactiveShortcutKeys.map((key) =>
  ...
  it("keeps A/D/W/S active when a workbench button still owns focus", () => {
    ...
    const actionEvents = ["a", "D", "w", "S"].map((key) =>
  ...
  it("ignores repeated toggle shortcuts while keeping frame navigation repeatable", () => {
    ...
    expect(dispatchKey(" ", window, { repeat: true }).defaultPrevented).toBe(
      true,
    );
    expect(dispatchKey("L", window, { repeat: true }).defaultPrevented).toBe(
      true,
    );
  ...
  it("keeps A/D repeatable while ignoring repeated W/S action shortcuts", () => {
    ...
    expect(dispatchKey("w", window, { repeat: true }).defaultPrevented).toBe(
      true,
    );
    expect(dispatchKey("S", window, { repeat: true }).defaultPrevented).toBe(
      true,
    );
  ...
  it("returns to the still-held direction when the other horizontal key is released", () => {
    const commands = createCommands();
    const unmount = mountKeyboard(commands);
    ...
    expect(commands.playLeft).toHaveBeenCalledTimes(2);
    expect(commands.playRight).toHaveBeenCalledOnce();
    expect(commands.stopHorizontal).not.toHaveBeenCalled();
    unmount();
  });
