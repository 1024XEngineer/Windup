...
describe("stage motion", () => {
  it("clamps at the right edge, turns left, and keeps moving inward", () => {
    ...
    expect(movedInward).toMatchObject({
      offset: { x: 7, y: 0 },
      mirrored: true,
    });
  });
  ...
  it("clamps at the left edge, turns right, and keeps moving inward", () => {
    ...
    const facingLeft = reduceStageMotion(bounded, {
      type: "set-mirrored",
      mirrored: true,
    });
  ...
  it("sets mirrored direction idempotently without resetting position and reverses future horizontal increments", () => {
    ...
    const mirrored = reduceStageMotion(movedRight, {
      type: "set-mirrored",
      mirrored: true,
    });
    const mirroredAgain = reduceStageMotion(mirrored, {
      type: "set-mirrored",
      mirrored: true,
    });
    ...
    const restored = reduceStageMotion(movedLeft, {
      type: "set-mirrored",
      mirrored: false,
    });

    expect(mirrored).toMatchObject({ offset: { x: 5, y: 1 }, mirrored: true });
    expect(mirroredAgain).toMatchObject({
      offset: { x: 5, y: 1 },
      mirrored: true,
    });
