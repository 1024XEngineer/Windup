...
describe("PLAYTEST_DEMO_CHARACTER", () => {
  it("provides a confirmed boy outfit with complete idle and walk fixtures", () => {
    ...
    expect(outfit!.actions.every((action) => action.sequences[0]?.frames.length === 8)).toBe(true);
    expect(outfit!.actions.every((action) => action.status === "confirmed")).toBe(true);

    const walkFrames = outfit!.actions.find((action) => action.type === "walk")!.sequences[0]!
      .frames;
    ...
    const frames = outfit!.actions.flatMap((action) => action.sequences[0]!.frames);
    expect(frames.some((frame) => frame.durationMs !== null)).toBe(true);
    expect(frames.some((frame) => frame.rootMotion !== null)).toBe(true);
    expect(frames.some((frame) => frame.qc === "passed")).toBe(true);
    expect(frames.some((frame) => frame.qc === "pending")).toBe(true);
    expect(frames.some((frame) => frame.rejected)).toBe(true);
