...
- 所有动作帧进入页面时统一预加载；切换动作只重置帧游标，不重复请求素材。普通动作按各自
  逐帧时长播放；只有生成器已知的 32 帧循环 walk/run 旧时序（每帧仍为 125ms/90ms）会按
  密集采样修正为 1000ms/720ms 的完整周期。帧数、循环性或任一时值不匹配时均尊重资产原值。
  角色位移仍使用 `requestAnimationFrame` 的真实时间差计算，不让动画帧率改变操控速度。
