...
export function createWorkflowEditorService({
  ...
  return {
    ...
    async continueWithUploadedTemplate(runId, file, actionDescription, signal) {
      if (!continueWithUploadedTemplate) {
        throw new Error('上传母版服务尚未配置')
      }
      return continueWithUploadedTemplate(runId, file, actionDescription, signal)
    },
