...
function toGenerationEvent(raw: BackendGenerationTask): GenerationEvent {
  const generation = toGeneration(raw)
  return {
...
export function createGenerationApis(): GenerationApis {
  return {
    ...
    subscribe(projectId, id, onEvent) {
      let active = true
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (!active) return
          onEvent(toGenerationEvent(raw))
          if (raw.status === 'completed' || raw.status === 'failed') return
        } catch {
          // 短暂网络失败时保留恢复能力，下一轮继续读取同一 Generation。
        }
