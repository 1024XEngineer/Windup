...
`api/` 默认从 `VITE_API_BASE_URL` 读取服务地址，并在发出请求时读取当前 access token。登录模块通过 `registerApiAccessTokenProvider` 提供 token，并通过 `registerApiUnauthorizedRecovery` 提供一次 401 恢复；公共层不持有认证状态。上传和普通 JSON 请求共用这套客户端。SSE 使用可设置 `Authorization` 的 fetch 流实现，不能使用浏览器原生 `EventSource`。
