...
**Architecture:** 保留后端和工作流控制器现有的逐方向 URL 合同。Quick Start 增加纯函数，把同一候选序号的真实生成方向结果组织成方向卡；UI 用 3×3 网格渲染真实 URL，四向斜向留空，确认时仍提交方向 URL 映射。单向继续使用现有单图选择路径。
...
- [ ] **Step 1: Write the failing test**
  - 覆盖八向生成 8 个真实方向时按候选序号生成卡片。
  - 覆盖四向只填东/西/北/南，四个斜向单元为 `empty`。
...
- [ ] **Step 3: Write minimal implementation**
  - 按 `getDirectionProfile(movement).generationDirections` 构造方向索引；按候选 `index` 对齐；只产出所有生成方向都有 URL 的卡片。
...
- [ ] **Step 3: Write minimal implementation**
  - 引入 `buildDirectionSheetCandidates`；多方向时渲染 3×3 卡片，空单元不渲染图片但保留布局。
