...
在此基础上,Windup 面向国产小游戏生态(微信 + Cocos)进行适配:导出资产按其技术规范组织,序列帧图集按远程加载目录排布,导入引擎即可播放;以角色母版约束保障跨动作、跨批次的视觉一致性,由工程系统而非模型概率输出保障;并保持开源、中文原生与生成模型 / 路线的可替换,不绑定单一厂商。
...
第二个空白在国内小游戏生态:据游戏工委年报,小程序游戏年收入 535 亿元,且全行业年度增长中过半来自小游戏。国内 2D 游戏的主要分发渠道为微信、抖音等小游戏平台;以微信为例,开发者规模约五十万,其中八成为三十人以下团队,开发管线高度集中于 Cocos 引擎(畅玩榜前 100 中 Cocos 占约 61%)。然而现有素材工具中,无一针对该生态进行适配。
...
- 命名收敛:原稿中"角色卡 / 角色身份 / 角色身份卡"三种称谓存在混用,现统一并入"角色"——"稳定信息 + 名下全部造型"的整体即为角色,身份为其属性,不再单独命名。
...
环境即"引擎 + 平台"(如 Cocos Creator + 微信小游戏),为项目的目标环境。就导出产物而言,不同环境的 PNG、图集、JSON 为同一份文件,差异仅在于开发者获取文件后的配置(如微信 4MB 主包与远程加载的分包组织),不属于本期工作;导出器按引擎以适配器组织,扩展引擎即新增一个导出目标,不影响产品。
...
6. **引擎适配**:导出包在 Cocos Creator / 微信小游戏首次播放成功,无需重切帧、批量改名、逐帧调锚点(假设 4);本期仅验收 Cocos 靶子,Godot、Unity 不在本期。
