...
def test_create_validation_error_returns_400(client):
    resp = client.post("/projects", json=_payload(project_name="x" * 65))
...
def test_get_success(client):
    created = client.post("/projects", json=_payload(project_name="详情")).json()[
        "data"
    ]
...
def test_delete_success(client):
    created = client.post("/projects", json=_payload(project_name="删除")).json()[
        "data"
    ]
...
def test_delete_cascades_characters_and_playtest_inspections(client):
    project = client.post("/projects", json=_payload(project_name="级联删除")).json()[
        "data"
    ]
    ...
    client.post(
        ...
        json={**inspection_target, "status": "passed"},
    )

    response = client.delete(f"/projects/{project['id']}")

    assert response.json()["data"] is True
    assert client.get(f"/characters/{character['id']}").json()["code"] == 404
    assert (
        client.get("/playtest-inspections", params=inspection_target).json()["code"]
        == 404
    )
