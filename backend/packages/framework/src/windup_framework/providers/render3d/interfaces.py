...
@dataclass(frozen=True)
class RigInfo:
    """出帧台从模型里读到的骨架事实。**记录用,不是判据**,也不参与渲染。

    曾写作「已确立(不必每次重验):自动绑骨产出 28 骨 · humanoid 命名 · 无 ``mixamorig:``
    前缀,对不上就该停下来看」。**重验一次即推翻**(2026-08-26,量全部归档绑骨产物):
    骨数是 24 / 27 / 28 / 43 / 49,其中 24 与 27 正是混元自己绑的四足 —— 同一条链路出来
    的骨数就不是常数;带 ``mixamorig`` 前缀的那个是 Mixamo 绑的,属重定向路线的正常输入。

    据此当闸会把自家四足产物与重定向输入一起挡掉。这几个数只进记录,不做拒绝。
    真正稳的前置条件只有一条:模型里得有 ``skins`` —— 没有骨架渲不出动画。
    """
