...
# 任务还显示成功。
#
# **这一份在 import 期读,而 build_*_prompt 在调用时读 —— 这不是策略不一致,是数据形态
# 不同**(FennoAI 问过,值得写下来):本项是模块级常量,``master_prep`` 又在生产路径上
# (``strategy.concrete`` import 它),所以 md 缺失会让 import 就炸。**那正是想要的**:
